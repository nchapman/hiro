---
name: delegate
description: Delegate work to sessions. Use when a task should be handled by a specialist or benefits from a focused context.
---
...
- It would take longer to explain to a session than to do it
...
**Use `create_session` + `send_message`** when:
...
## Writing good prompts for sessions

The session has no context beyond what you give it. Every prompt should be self-contained:
...
Each `spawn_session` call blocks until the session finishes. For large tasks, break the work into independent chunks and spawn one agent per chunk:
...
When you get results back from sessions:
...
- If a session fails, try to understand why before retrying. Read the error.
...
- Use `list_sessions` to see your direct child agents before starting duplicates. (It doesn't show grandchildren — agents started by your children.)
