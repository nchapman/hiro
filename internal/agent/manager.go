...
type Manager struct {
	...
	landlockEnabled bool                        // Landlock available on this kernel
...
// landlockEnabled indicates whether Landlock filesystem restrictions are available.
func NewManager(ctx context.Context, rootDir string, opts Options, cp ControlPlane, logger *slog.Logger, wf WorkerFactory, pdb *platformdb.DB, landlockEnabled bool) *Manager {
	if wf == nil {
