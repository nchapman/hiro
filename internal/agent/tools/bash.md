...
- Set `run_in_background` for intentionally long-running processes, then manage with `BashOutput` and `TaskStop`
...
- Don't `sleep` to wait for background work — use `BashOutput` with `wait` instead
