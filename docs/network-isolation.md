...
Agents without a `network` field in their definition are placed in a network namespace with an empty IP set and no DNS forwarding. The nftables chain allows only DNS to the gateway (which returns NXDOMAIN for everything) and drops all other traffic. Effectively zero connectivity. This is the default-deny posture — agents must explicitly declare `network.egress` to get outbound access.
...
3. **CNAME handling:** DNS responses may include CNAME chains (e.g., `github.com → github.com.cdn.fastly.net → 151.101.1.194`). The forwarder collects all terminal A/AAAA records from the full answer section. Intermediate CNAME targets are validated against the agent's `network.egress` allowlist — if any CNAME in the chain points to a domain outside the allowlist, the entire response is rejected (SERVFAIL) to prevent allowlist bypass via CNAME redirection. This means operators may need to allowlist CDN or other intermediary CNAME domains used by an otherwise allowed destination. Wildcard egress (`["*"]`) skips CNAME validation.
...
- **No `network` field**: No outbound connectivity. Agent can only use Unix sockets (gRPC to control plane). This is the default — agents must explicitly declare what network access they need.
